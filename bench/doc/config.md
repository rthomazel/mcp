...
| `BENCH_MCP_BACKGROUND_NICE`    | `10`      | CPU scheduling priority (nice level 0–20) applied to `shell_background` jobs. Keeps heavy background jobs from starving foreground `shell` commands. `0` disables the wrapper. Does not bound memory or the aggregate CPU of several simultaneous jobs. |
